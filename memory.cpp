...
bool Call64(HANDLE process, void *function, DWORD argument_count, ...) {
...
void *FindPattern(void *base, unsigned int length, char *pattern, char *mask) {
...
void *ProcessFindPattern(HANDLE process, void *base, unsigned int length, char *pattern, char *mask) {
	char *buffer = (char *)malloc(length);
	if (!buffer) return 0;

	SINT addr = 0;
	if (ReadProcessMemory(process, base, buffer, length, 0)) {
		addr = (SINT)FindPattern(buffer, length, pattern, mask) - (SINT)buffer + (SINT)base;
	}
